...
const WEB3FORMS_ACCESS_KEY = "4e92a331-2386-417d-ad39-6f3c17dee60d";

document.addEventListener("DOMContentLoaded", () => {
  // --- NAME FIELD VALIDATION ---
  document
    .querySelectorAll(
      'input[name="name"], #inq-name, #car-name, #inq-name-modal, #car-name',
    )
    .forEach((input) => {
      input.addEventListener("input", (e) => {
        ...
        if (filtered !== originalValue) {
          ...
          showToast(
            "Only alphabets, dots, and spaces are allowed in the name field.",
            true,
          );
        }
      });
  ...
  if (inquiryForm) {
    inquiryForm.addEventListener("submit", async (e) => {
      ...
      const formData = {
        ...
        phone,
        service:
          inquiryForm.querySelector('[name="service"]')?.value ||
  ...
  if (newsletterForm) {
    newsletterForm.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        showToast(
          "Subscribed! You'll receive compliance updates in your inbox.",
        );
  ...
  async function simulateSend(data) {
    if (
      ...
      console.warn(
        "[MSR Mailer] Web3Forms Access Key is not configured. Simulating submission.",
      );
